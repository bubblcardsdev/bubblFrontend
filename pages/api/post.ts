...
async function Handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (isSuccess) {
        console.log(response.data.orderType, "__/_/_/_/_/_/_/_/_/__/_/_/_/_/_____________________");

        switch (response.data.orderType) {
            case "0":
                res.writeHead(302, { Location: `/order_details?orderId=${orderId}` });
                return res.end(); 

            case "2":
                res.writeHead(302, { Location: `/paymentResponse?orderId=${orderId}` });
                return res.end(); 

            default:
                const token = response.data.jwtToken;
                const planObj = { planId: 2, planValidity: response.data.planType };
